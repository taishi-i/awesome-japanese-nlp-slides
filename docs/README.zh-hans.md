...
共收录 665 份资料，分为 32 个分类。

幻灯片标题保留发表时的原文，因此绝大多数为日语。

> [!NOTE]
> 本列表持续更新中，会不断新增幻灯片、充实内容。
...
- [NLP 库与工具开发](#nlp-库与工具开发)
- [日语文本的规范化・写法差异](#日语文本的规范化写法差异)
- [词典・假名汉字转换・日语输入（IME）](#词典假名汉字转换日语输入ime)
- [词与句的分布式表示](#词与句的分布式表示)
...
- [AI 智能体・MCP](#ai-智能体mcp)
- [LLM 的安全性・安全防护](#llm-的安全性安全防护)
- [文本分类・情感分析](#文本分类情感分析)
- [垃圾信息・有害内容治理](#垃圾信息有害内容治理)
- [文本挖掘・主题模型](#文本挖掘主题模型)
...
🙋 有想收录的幻灯片吗？请参考[贡献指南](https://github.com/taishi-i/awesome-japanese-nlp-slides/blob/main/contributing.md)提交 Pull Request。

## 🔍 从 Claude Code 搜索

我们提供了插件，让你可以从 Claude Code 搜索本列表收录的 665 份资料。
...
🌐 企业与个人开发者发布的博客文章，另外整理于[这份文章列表](https://github.com/taishi-i/awesome-japanese-nlp-slides/blob/main/docs/ARTICLES.zh-hans.md)（183 篇）。
...
假名汉字转换引擎与输入法（IME）的实现，以及作为其基础的词典与词汇资源的构建。
...
让 LLM 使用工具的智能体的设计、评估与运维，以及通过 MCP 实现的工具联动。
...
发现并清除垃圾投稿、违规内容与虚假信息的技术，以及平台运营的做法。
...
对问卷与评论等自由文本进行计量分析的方法，以及用主题模型挖掘潜在结构。
